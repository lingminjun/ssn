...
- (NSArray *)queryObjects:(Class)aclass
                      sql:(NSString *)sql, ...; //参数必须传入object对象,无法执行DDL操作，仅仅支持DML操作,请以nil结尾
- (void)queryObjects:(Class)aclass completion:(void (^)(NSArray *results))completion sql:(NSString *)sql, ...;
- (NSArray *)queryObjects:(Class)aclass
                      sql:(NSString *)sql
                arguments:(NSArray *)arguments; //参数必须传入object对象,无法执行DDL操作，仅仅支持DML操作,请以nil结尾
...
- (NSArray *)dataBase:(SSNDataBase *)database
    columnsForTemplateName:(NSString *)templateName
