...
class CPackData
{
  ...
  public:
    ...
    template <typename _Tp> CPackData &operator>>(std::vector<_Tp> &aryVal)
    {
        uint32_t dwLen;
        *this >> dwLen;
    ...
    template <typename _Tp> bool ReadData(std::vector<_Tp> &aryVal, int nMode)
    {
        uint32_t nLen;
        switch (nMode)
        {
    ...
    bool WriteData(const std::string &strVal, int nMode, uint32_t nMaxLen = 0, char chFill = 0)
    {
        ...
        case 0:
            ...
            {
                m_outPtr->replace(m_outCursor, strVal.length(), strVal);
                m_outPtr->replace(m_outCursor + strVal.length(), nMaxLen - strVal.length(), nMaxLen - strVal.length(),
                                  chFill);
            }
            else
    ...
    template <typename _Tp> CPackData &operator<<(const std::vector<_Tp> &aryVal)
    {
    ...
    CPackData &operator>>(std::map<std::string, std::string> &attrs)
    {
        CFieldType field;
        (*this) >> field;
        if (field.m_baseType != FT_MAP)
            throw PACK_TYPEMATCH_ERROR;
        {
            uint8_t nSize;
            (*this) >> nSize;
    ...
    template <typename _Tp> bool WriteData(const std::vector<_Tp> &aryVal, int nMode)
    {
        switch (nMode)
        {
        case 1:
