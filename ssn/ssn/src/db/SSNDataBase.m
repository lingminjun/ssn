...
- (NSArray *)tableColumnsWithTemplateName:(NSString *)tableName
...
- (dispatch_queue_t)ioQueue
{
    ...
    dispatch_queue_set_specific(_ioQueue, (__bridge const void *)self.queueSpecific,
                                (__bridge void *)self.queueSpecific, NULL);
    return _ioQueue;
}
...
+ (NSString *)insureThePath:(NSString *)thePath
{
    ...
    if (![thePath hasPrefix:application_Home])
    { //路径是合理的
...
- (void)createTableVersionLogTable
{
    NSArray *cols = [NSArray arrayWithObjects:[SSNTableColumnInfo columnWithName:kSSNDBTableName
...
{
    NSArray *cols =
        [NSArray arrayWithObjects:[SSNTableColumnInfo columnWithName:kSSNDBTableTemplateName // version-TableName
                                                                type:SSNModelPropertText
                                                             keyType:SSNModelPropertPrimaryKey
                                                           indexType:SSNModelPropertNormalIndex
                                                             default:@""
...
- (void)saveTableColumns:(NSArray *)cols forTemplateName:(NSString *)tableName dataBaseVersion:(NSInteger)version
{
    NSString *sql =
        [NSString stringWithFormat:@"INSERT OR REPLACE INTO %@ (%@,%@,%@,%@,%@,%@,%@) VALUES(?,?,?,?,?,?,?)",
                                   ...
                                   // begin values
                                   kSSNDBTableTemplateName, kSSNDBTableColumnName, kSSNDBTableColumnType,
                                   kSSNDBTableKeyType, kSSNDBTableIndexType, kSSNDBTableDefaultValue, kSSNDBTableMaping
                                   // end
    ];
    NSString *v_tbaleName = [NSString stringWithFormat:@"%ld-%@", version, tableName];

    [self executeSync:YES
        inTransaction:^(SSNDataBase *dataBase, BOOL *rollback) {
                        ...
                        prepareSql:sql
                         arguments:[NSArray
                                       arrayWithObjects:v_tbaleName, col.column, [NSNumber numberWithInt:col.type],
                                                        ...
                                                        (col.mappingFormatter ? col.mappingFormatter : [NSNull null]),
                                                        nil]];
            }
        }];
}
...
- (NSArray *)tableColumnsWithTemplateName:(NSString *)tableName dataBaseVersion:(NSInteger)version
{
    ...
    for (NSDictionary *row in cols)
    {
        ...
        SSNModelPropertIndexType indexType =
            (SSNModelPropertIndexType)[[row objectForKey:kSSNDBTableIndexType] integerValue];
...
- (void)createTable:(NSString *)tableName withDelegate:(id<SSNModelTableProtocol>)delegate
{
    ...
    dispatch_block_t block = ^{
        @autoreleasepool
        {
            ...
            if (dbVersion >= self.currentVersion)
            {                                    //不需要更新
            ...
            NSArray * (^t_block)(NSUInteger version, NSArray * lastAry, BOOL saveColumns) =
                ^(NSUInteger version, NSArray *lastAry, BOOL saveColumns) {
...
- (NSArray *)queryObjects:(Class)aclass prepareSql:(NSString *)sql arguments:(NSArray *)arguments
{
    ...
    if (sqlite3_prepare_v2(_database, [sql UTF8String], -1, &statement, NULL) == SQLITE_OK)
    {
        ...
        else
        {
            ...
            [self raiseSqliteException:[[NSString stringWithFormat:@"Failed to bind arguments: '%@' with message: ",
                                                                   sql] stringByAppendingString:@"%S"]];
        }
    }
    else
    {
        [self raiseSqliteException:[[NSString stringWithFormat:@"Failed to execute statement: '%@' with message: ", sql]
...
- (void)queryObjects:(Class)aclass
...
{
    NSString *sql = [NSString stringWithUTF8Format:"%s %s %s DEFAULT %s", [self.column UTF8String],
                                                   [[SSNTableColumnInfo columnTypeToString:self.type] UTF8String],
                                                   [[SSNTableColumnInfo columnKeyTypeToString:self.keyType
                                                                            supportPrimaryKey:!mutable] UTF8String],
...
- (NSString *)createIndexSQLStringWithTableName:(NSString *)tableName
{
    ...
    NSString *sql = [NSString stringWithUTF8Format:"CREATE %s INDEX IF NOT EXISTS idx_%s_%s ON %s(%s)",
                                                   ...
                                                   [tableName UTF8String], [self.column UTF8String],
                                                   [tableName UTF8String], [self.column UTF8String]];
    return sql;
}
...
{
    if ([self.mappingFormatter length])
    { //需要迁移,直接as就好了
        return
            [NSString stringWithUTF8Format:"(%s) AS %s", [self.mappingFormatter UTF8String], [self.column UTF8String]];
    }
    else
    {
        if (exist)
        ...
        {
            return [NSString
                stringWithUTF8Format:"%s AS %s", [[self defaultValueString] UTF8String], [self.column UTF8String]];
        }
    }
}
...
- (BOOL)isEqualToColumnInfo:(SSNTableColumnInfo *)col ignoreMapping:(BOOL)ignoreMapping
{
    if ([self.column isEqualToString:col.column] &&
        (([self.defaultValue length] == 0 && [col.defaultValue length] == 0) ||
         [self.defaultValue isEqualToString:col.defaultValue]) &&
        self.type == col.type && self.keyType == col.keyType && self.indexType == col.indexType)
    {
        if (ignoreMapping)
        {
            return YES;
        ...
        {
            if (([self.mappingFormatter length] == 0 && [col.mappingFormatter length] == 0) ||
                [self.mappingFormatter isEqualToString:col.mappingFormatter])
            {
                return YES;
            }
            else
