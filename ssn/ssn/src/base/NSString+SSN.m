...
- (NSString *)urlDecode {
    NSString *resultString = self;
    NSString *temString = CFBridgingRelease(CFURLCreateStringByReplacingPercentEscapesUsingEncoding(kCFAllocatorDefault,
                                                                                                                   (CFStringRef)self,
                                                                                                                   CFSTR(""),
                                                                                                                   kCFStringEncodingUTF8));
...
- (NSRange)lastNestSubstringRange {
    ...
    do {
        ...
        if (splitRange.length == 0) {//没有找到
            break;
        }
        
        if (!nestFlag) {//说明是后面的回引号
