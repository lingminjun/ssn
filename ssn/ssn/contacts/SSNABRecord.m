...
{
	return [[SSNABRecord alloc] initWithRecord:person];
}
...
//    dispatch_release(sema);
	...
	SSNABRecord *contact = [self contactWithRecord:contactrec];
...
+ (id) contact
{
	...
	id contact = [SSNABRecord contactWithRecord:person];
	CFRelease(person);
	return contact;
}
...
- (BOOL) removeSelfFromAddressBook: (NSError **) error
{
    ...
    if (!ABAddressBookRemoveRecord(addressBook, self.record, &aerr)) {
        if (error) {
            *error = (__bridge NSError *)(aerr);
        }
        return NO;
    }
	bool result = ABAddressBookSave(addressBook,  &aerr);
    if (!result) {
        if (error) {
            *error = (__bridge NSError *)(aerr);
        }
...
{
    NSString *reslut = nil;
	CFStringRef string = ABRecordCopyCompositeName(record);
    if (string) {
        reslut = [NSString stringWithString:(__bridge NSString *)(string)];
        CFRelease(string);
    }
	return reslut;
}
...
- (NSArray *) labelsForProperty: (ABPropertyID) anID
{
	...
	for (int i = 0; i < ABMultiValueGetCount(theProperty); i++)
	{
        ...
        if (label) {
            [labels addObject:[NSString stringWithString:(__bridge NSString *)(label)]];
            CFRelease(label);
        }
	}
	CFRelease(theProperty);
...
- (BOOL) setString: (NSString *) aString forProperty:(ABPropertyID) anID
{
	...
	if (!success) SSNLogError(@"%@", [(__bridge NSError *)error localizedDescription]);
	return success;
}
...
{
	CFErrorRef error;
	BOOL success = ABRecordSetValue(record, anID, (__bridge CFNumberRef) aNumber, &error);
	if (!success) SSNLogError(@"%@", [(__bridge NSError *)error localizedDescription]);
	return success;
}
...
- (ABMutableMultiValueRef) createMultiValueFromArray: (NSArray *) anArray withType: (ABPropertyType) aType
{
    ...
    for (NSDictionary *dict in anArray) {
        ABMultiValueAddValueAndLabel(multi,
                                     (__bridge CFTypeRef) [dict objectForKey:@"value"],
                                     (__bridge CFTypeRef) [dict objectForKey:@"label"],
                                     NULL);
    }
...
{
	NSMutableDictionary *dict = [[self baseDictionaryRepresentation] mutableCopy];
	if (ABPersonHasImageData(record))
	{
		CFDataRef imageData = ABPersonCopyImageData(record);
		[dict setObject:(__bridge NSData *)imageData forKey:IMAGE_STRING];
		CFRelease(imageData);
	}
	return dict;
}
...
- (NSData *) baseDataRepresentation
{
	...
	if (!data) CFShow((__bridge CFTypeRef)(errorString));
	return data;
}
...
{
	SSNABRecord *contact = [SSNABRecord contact];
	...
	{
		CFErrorRef error;
 		BOOL success = ABPersonSetImageData(contact.record, (__bridge CFDataRef) [dict objectForKey:IMAGE_STRING], &error);
		if (!success) SSNLogError(@"%@", [(__bridge NSError *)error localizedDescription]);
	}
...
+ (id) contactWithData: (NSData *) data
{
	...
	NSDictionary *dict = [NSDictionary dictionaryWithDictionary:(__bridge NSDictionary *)(plist)];
    CFRelease(plist);
